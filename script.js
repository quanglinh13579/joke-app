...
let currentIndex = getNextJokeIndex();

function getNextJokeIndex() {
  for (let i = 0; i < jokes.length; i++) {
    if (!voted.includes(i)) return i;
  }
  return -1;
}
...
function vote(type) {
  console.log(`Voted ${type} for joke #${currentIndex}`);
  voted.push(currentIndex);
